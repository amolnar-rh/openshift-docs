...
. Verify that the required `baremetal-event-relay` hardware event producer pod is running in the `openshift-baremetal-events` namespace:
+
[source,terminal]
----
$ oc get pods -n openshift-baremetal-events
----
+
.Example output
[source,terminal]
----
NAME                                                         				 READY   STATUS    RESTARTS   AGE
baremetal-event-relay-controller-manager-b889bb8d5-qwhvs   2/2     Running   0          25s
----
