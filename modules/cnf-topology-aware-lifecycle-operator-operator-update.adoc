...
<2> Set how frequently OLM pulls the index image for new Operator versions with the `registrypoll.interval` field. This change is not needed if a new index image tag is always pushed for y-stream and z-stream Operator updates.
+
...
.. Save the content of the `cgu-operator-upgrade-prep` `ClusterGroupUpgrade` CR with the catalog source policies and the target spoke clusters to the `cgu-operator-upgrade-prep.yml` file.
+
[source,yaml]
----
...
<2> The policy contains the Operator subscriptions. If you have upgraded ZTP from 4.9 to 4.10 by following the "Upgrading GitOps ZTP" procedure, all Operator subscriptions should be grouped into the `common-subscriptions-policy` policy.
+
[NOTE]
====
...
.. Verify that the subscription policy is `NonCompliant` at this point by running the following command:
+
[source,terminal]
----
...
[source,terminal]
----
$ oc get cgu cgu-upgrade-complete -ojson | jq '.status.conditions'
----
+
.Example output
+
[source,json]
----
- lastTransitionTime: 2022-03-08T20:55:30Z
