...
If you have deployed spoke clusters with distributed unit (DU) profiles using the GitOps ZTP with the Topology Aware Lifecycle Operator pipeline described in "Deploying distributed units at scale in a disconnected environment", this procedure describes how to upgrade your spoke clusters and Operators.
...
The TALO can perform both platform and Operator updates.

You must mirror both the platform image and Operator images that you want to update to in your mirror registry before you can use the TALO to update your disconnected clusters. Complete the following steps to mirror the images:
...
. Save the output of the `imageContentSources` section in a file. Retrieve the contents of the `imageContentSources` section, by running the following command:
+
[source,terminal]
----
...
[NOTE]
====
Save the image signature of the desired platform image that was mirrored. You must add the image signature to the `PolicyGenTemplate` CR for platform updates.
====

.. To get the image signature, first, specify the desired {product-title} tag by running the following command:
+
[source,terminal]
----
