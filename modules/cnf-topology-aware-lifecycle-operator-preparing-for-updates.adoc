...
+
. Mirror the desired {product-title} image repository. Ensure that the desired platform image is mirrored by following the "Mirroring the OpenShift Container Platform image repository" procedure. Save the contents of the `imageContentSources` section in the `imageContentSources.yaml` file. Retrieve the contents of the `imageContentSources` section, by running the following command:
+
[source,terminal]
----
