...
Topology Aware Lifecycle Operator (TALO) builds the remediation plan from the `ClusterGroupUpgrade` CR for a group of clusters. You can define the following specifications in a `ClusterGroupUpgrade` CR:
...
As TALO works through remediation of the policies to the specified clusters, the `ClusterGroupUpgrade` CR can have the following states:
...
[NOTE]
====
After TALO completes a cluster update, the cluster does not update again under the control of the same `ClusterGroupUpgrade` CR. You must create a new `ClusterGroupUpgrade` CR in the following cases:
...
In the `UpgradeTimedOut` state, TALO checks every hour that all the policies for the `ClusterGroupUpgrade` CR are compliant. The checks continue until the `ClusterGroupUpgrade` CR is deleted or the updates are completed.
The periodic checks allow the updates to complete if prolonged due to network, CPU, or other issues.

TALO transitions to the `UpgradeTimedOut` state in the following cases:

* When the current batch is of `canary` type and the cluster in the batch does not comply with all the managed policies within the batch timeout.
...
status: <2>
  ...
  status:
    remediationPlanForBatch:
      spoke1: -2 <3>
----
<1> The value of `spec.action.afterCompletion.deleteObjects` field is `true` by default. After the update is completes, TALO deletes the underlying {rh-rhacm} objects that were created during the update. This option prevents the {rh-rhacm} Hub from continuously checking for compliance after a successful update.
