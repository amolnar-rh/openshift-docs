...
[source,terminal]
----
$ oc get cgu -n default cgu-1 -ojsonpath='{.status}'
----
+
.Example output
+
...
. If the policies include Operator subscriptions, you can checnk the installation progress directly on the single node cluster.

.. Export the `KUBECONFIG` of the single node cluster you want to check the installation progress for by running the following command:
+
[source,terminal]
----
$ export KUBECONFIG=<absolute path to the cluster kubeconfig>
----
...
[source,terminal]
----
oc get subs -A | grep -i <name of the subscription you are looking for>
----
+
...
[source,terminal]
----
$ oc get installplan -n <subscription namespace>
----
+
...
----
<1> The install plans have their `Approval` field set to `Manual` and their `Approved` field changes from `true` to `false` after  the TALO approves the install plan.
