4-10// Module included in the following assemblies:
...
<3> Shows the mirror repository that contains the desired {product-title} image. Get the mirrors from the `imageContentSources.yaml` you saved when following the procedures in the "Setting up the environment" section.
<4> Shows the source repository that the desired {product-title} image is pulled from. You should use the `imageContentSources` listed in the `oc adm release mirror` command output here.
...
.. Save the content of the platform update `ClusterGroupUpdate` CR with the `du-upgrade-platform-upgrade` policy and the target clusters to the `cgu-platform-upgrade.yml` file, as shown in the example:
+
[source,yaml]
----
apiVersion: ran.openshift.io/v1alpha1
