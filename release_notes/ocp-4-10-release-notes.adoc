...
==== Bare Metal Event Relay (Technology Preview)

{product-title} now provides a hardware event proxy that enables applications running on bare-metal clusters to respond quickly to Redfish hardware events, such as hardware changes and failures. The Bare Metal Event Relay Operator manages the lifecycle of the hardware event proxy container. A bare-metal events REST API for application development is also available.

The hardware event proxy supports a publish-subscribe service that allows applications to consume hardware events detected by Redfish. The proxy must be running on hardware that supports Redfish v1.8 and higher.

Reliable end-to-end messaging is based on the Advanced Message Queuing Protocol (AMQP). The latency of the messaging service is in the 10 millisecond range.

[NOTE]
====
The Bare Metal Event Relay is supported for single-node OpenShift clusters only.
====

For more information, see xref:../monitoring/using-rfhe.adoc#using-rfhe[Monitoring bare-metal events with the Bare Metal Event Relay].
...
If a storage volume contains many files (approximately 1,000,000 or greater), you may experience pod timeouts.
